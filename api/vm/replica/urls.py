...
urlpatterns = [
    ...
    url(r'^api/vm/(?P<hostname_or_uuid>[A-Za-z0-9\._-]+)/replica/$', vm_replica_list, name='api_vm_replica_list'),
    # /vm/<hostname_or_uuid>/replica/<repname> - get, create, set, delete
    url(r'^api/vm/(?P<hostname_or_uuid>[A-Za-z0-9\._-]+)/replica/(?P<repname>[A-Za-z0-9\._-]+)/$', vm_replica,
        name='api_vm_replica'),
