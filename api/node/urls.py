...
urlpatterns = patterns(
    ...
    url(r'^(?P<hostname>[A-Za-z0-9\._-]+)/storage/(?P<zpool>[A-Za-z0-9\._-]+)/image/$',
        'node_image_list', name='api_node_image_list'),
    ...
    url(r'^(?P<hostname>[A-Za-z0-9\._-]+)/storage/(?P<zpool>[A-Za-z0-9\._-]+)/snapshot/$',
        'node_vm_snapshot_list', name='api_node_vm_snapshot_list'),
...
if settings.MON_ZABBIX_ENABLED:
    urlpatterns += patterns('', url(r'^', include('api.mon.node.urls')))
