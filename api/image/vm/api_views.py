...
class ImageVmView(APIView):
    """
    api.image.vm.views.image_vm_list
    """
    dc_bound = False
    ...
    def get(self, many=True):
        """Display list of VMs that use a specific image"""
        assert many  # Currently only used for displaying list of VMs

        request, img = self.request, self.img
        ...
        vms = [vm for vm in get_vms(request, sr=sr, order_by=self.order_by, dc__in=img.dc.all(), slavevm__isnull=True)
               if img.uuid in vm.get_image_uuids()]
