...
VMS_DISK_IMAGE_ZONE_DEFAULT = 'base-64-es'
...
VMS_VM_ZONE_USER_SCRIPT_DEFAULT = """if [ ! -f /var/svc/provision_esdc ]; then
  ...
  /usr/sbin/mdata-get deploy-script > /var/svc/user-deploy-script && /usr/bin/bash /var/svc/user-deploy-script
  {image_deploy}
fi
/usr/sbin/mdata-get startup-script > /var/svc/user-startup-script && /usr/bin/bash /var/svc/user-startup-script
exit 0
"""
